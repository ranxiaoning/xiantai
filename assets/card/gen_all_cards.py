...
SIZE_NAME = 95   # 卡牌名称的字体大小
SIZE_TYPE = 60   # 卡牌类型的字体大小
SIZE_DESC = 88   # 卡牌描述文本的字体大小
...
def centered(draw, text, font, cx, cy, color, shadow=(0, 0, 0, 200)):
    ...
    # 再在原位置绘制文本主体（stroke 描边加粗轮廓提升清晰度）
    draw.text((x, y), text, font=font, fill=color, stroke_width=2, stroke_fill=shadow)
...
def render_card(card):
    ...
    f_desc = ImageFont.truetype(FONT_BOLD,  SIZE_DESC)   # 黑体加粗，描述更清晰
    ...
    if os.path.exists(art_path):
        try:
            ...
            art_img = art_img.resize((1080, 1020))
            # 将原画粘贴到模板图上，坐标为 (270, 450)，使用自身作为 mask 处理透明边缘
            img.paste(art_img, (215, 425), art_img)
